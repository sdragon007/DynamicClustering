...
PointNetwork::PointNetwork() : Network() {
    numPoints = 0;
}

PointNetwork::PointNetwork(char *fileName, mt19937* mr, bool std):Network::Network(fileName) {
    randGen = mr;
...
int PointNetwork::randomIndex() {
    return (*randGen)() % numPoints;
}

int PointNetwork::randomIndex(set <int> &indexToExclude) {
    int i;
    while (1) {
...
vector <Point*> PointNetwork::randomPoints(int n) {
    set <int> s;
    int i;
    vector <Point*> ans;
    while (ans.size()<n) {
